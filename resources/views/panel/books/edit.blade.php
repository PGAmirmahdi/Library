...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form id="book-form" action="{{ route('books.update', $book->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-row">
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-3">
                        <label for="name">عنوان کتاب<span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control" id="name" value="{{ old('name', $book->name) }}">
                        @error('name')
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-3">
                        ...
                        @enderror
                        @if($book->image)
                            <img src="{{ asset('storage/' . $book->image) }}" alt="تصویر کتاب" style="max-width: 100px; margin-top: 10px;">
                        @endif
                    </div>
                </div>
                <div class="col-xl-12 col-lg-12 col-md-12 mb-3">
                    <label for="description">توضیحات</label>
                    <textarea name="description" class="form-control textarea" id="description">{{ old('description', $book->description) }}</textarea>
                    @error('description')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                <div class="table-responsive">
                    <table class="table table-bordered table-striped text-center" id="features_table">
                        ...
                        <tbody>
                        @foreach(json_decode($book->features) as $feature)
                            <tr>
                                <td>
                ...
                <div class="form-row">
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-3">
                        ...
                        <input type="number" name="inventory" class="form-control" id="inventory" value="{{ old('inventory', $book->inventory) }}">
                        @error('inventory')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-3">
                        ...
                        <input type="number" name="total_count" class="form-control" id="total_count" value="{{ old('total_count', $book->total_count) }}">
                        @error('total_count')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                </div>
                <button class="btn btn-primary" type="submit">به‌روزرسانی کتاب</button>
            </form>
        </div>
    </div>
